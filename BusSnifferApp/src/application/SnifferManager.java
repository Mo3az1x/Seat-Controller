...
public class SnifferManager implements SerialComm.DataSink, AutoCloseable {
    ...
    // Buffer to store last Read All dump
    private byte[] lastReadAll;
    ...
    private void handleFrame(byte[] data, int len) {
        ...
        switch (cmdId) {
            ...
            case RES_READ_ALL:
                int size = (len - 7);
                lastReadAll = new byte[size];
                System.arraycopy(data, 6, lastReadAll, 0, size);
                msg = "READ_ALL Response: size=" + size + " bytes";
                break;
    ...
    public void sendRaw(byte[] bytes) {
        if (bytes != null && bytes.length > 0) {
            serial.send(bytes);
            log("Sent RAW: " + hexLine(bytes, bytes.length));
        }
    }

    // 🆕 Getter for last Read All (Profiles)
    public byte[] getLastReadAll() {
        return lastReadAll;
    }

    public void saveProfile(String string) {
